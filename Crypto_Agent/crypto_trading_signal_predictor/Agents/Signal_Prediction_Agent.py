...
with open("Instructions/signal_predictor_agent_instructions.md", "r") as file:
    signal_predictor_agent_instructions = file.read()
...
signal_predictor_agent = Agent(
    name="Prediction Agent",
    instructions=signal_predictor_agent_instructions,
    tools=[
...
async def run_agent(conversation_text, intent, pair):
    ...
    while current_key_index < len(GEMINI_KEYS):
        ...
        signal_predictor_agent.model = model

        try:
            result = await Runner.run(
                starting_agent=signal_predictor_agent,

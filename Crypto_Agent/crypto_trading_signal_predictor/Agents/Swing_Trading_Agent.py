...
from tools.swing_trading_tool import get_swing_trade_tool


# --- Load environment variables ---
load_dotenv()

with open("Instructions/swing_trading_agent.md", "r") as file:
    swing_trading_agent_instructions = file.read()
...
swing_trade_agent = Agent(
    name="Swing Trade Agent",
    instructions=swing_trading_agent_instructions,
    tools=[
        get_swing_trade_tool
    ],
    model=None  
)
...
async def run_agent(conversation_text, intent, pair):
    ...
    while current_key_index < len(GEMINI_KEYS):
        ...
        swing_trade_agent.model = model

        try:
            result = await Runner.run(
                starting_agent=swing_trade_agent,
